...
data = data.dropna(subset=['cool'])
data = data[data['cool'] != '']
data['cool'] = pd.to_numeric(data['cool'])  # Convert 'cool' column to numeric if it's not already
...
labels = torch.tensor(data['cool'] - 1)  # Subtract 1 to convert labels to start from 0 for classification
...
with open('neural_network_model_project2_cool.pkl', 'wb') as f:
    pickle.dump(model, f)
